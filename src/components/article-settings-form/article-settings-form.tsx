...
export default function ArticleSettingsForm({tagList, submitText, error, onSuccessSubmit,  inputField,  header, article}:IArticleSettingsFormProps) {

    const {register, handleSubmit, formState: {errors}, control, reset} = useForm<IArticleForm>({
        mode: "onBlur",
            ...
            <>
                    ...
                    {inputField.map((field) => {
                            return (
                                <li key={field.name} className="article-settings-form__input-list-item">
                                    <InputForm errors={errors} register={register}  {...field} defaultValue={article?.[field.name] || ''}
                                                                              cl={`article-settings-form__input ${field.name==='text' ? "article-settings-form__input-text" : ''}`} />
                ...
                <div style={{display: "flex", alignItems: "flex-end", marginBottom: "21px"}}>
                    <ul className="article-settings-form__tag-list">
                        <div>Tags</div>
                        { fields.map((field, i, list) => (
                            <li key={field.id} className="article-settings-form__tag-list-item">
                                <InputForm {...tagField} name={`tags.${i}.tag`} defaultValue={article?.tagList?.[i] || ''}  errors={errors} register={register} cl={"article-settings-form__tag-list-input"}/>
                                <Button color="error" className="article-settings-form__tag-list-btn" variant="outlined" onClick={()=> remove(i)}>Delete</Button>
                            </li>
                        ))}
                    </ul>
                    <Button className="article-settings-form__tag-list-btn" sx={{width: "fit-content"}} color="info" variant="outlined" onClick={()=> append({tag: ""})}>Add tag</Button>
                </div>

                {error || null}
                <Button component="button" type="submit" variant="contained" color="primary"
                        className="article-settings-form__submit"
                        sx={{width: '320px'}}>{submitText}</Button>
